...
function setup_vendor() {
    ...
    export DELUXE_ROOT="$3"
    if [ ! -d "$DELUXE_ROOT" ]; then
        echo "\$DELUXE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$DELUXE_ROOT/$OUTDIR" ]; then
        mkdir -p "$DELUXE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$DELUXE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$DELUXE_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$DELUXE_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$DELUXE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR DeluxeOS\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR DeluxeOS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR DeluxeOS\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2019 ]; then
            printf "# Copyright (C) 2019 DeluxeOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR DeluxeOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
            printf "# Copyright (C) 2019-$YEAR DeluxeOS\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR DeluxeOS\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR DeluxeOS\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$DELUXE_ROOT"/prebuilts/tools-deluxe/common/smali/baksmali.jar
        export SMALIJAR="$DELUXE_ROOT"/prebuilts/tools-deluxe/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$DELUXE_ROOT"/prebuilts/tools-deluxe/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$DELUXE_ROOT"/prebuilts/tools-deluxe/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$DELUXE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$DELUXE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$DELUXE_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$DELUXE_ROOT"/vendor/deluxe/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$DELUXE_ROOT"/"$OUTDIR"/radio
